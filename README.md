# 창문형 실시간 복사열 감지 및 가변형 자동 단열 가림막
...
- 가림막 이동 경로를 비우고 Nano가 MANUAL인지 확인한 뒤 외부 5 V를 연결합니다.
- 입력: `HOME`.
...
- 즉시 중단: 물리적 하단 막음캡 충돌, 줄/가림막 미끄러짐, 위치 오차 급증.
- 해결: 다시 HOME/CALIBRATE하고 장력·커플러·축 미끄러짐을 수정합니다.

### TEST 9 — 센서 데이터

- 입력: `LOG ON`.
- 조건: 직사광, 약한 햇빛, 그늘을 차례로 만들고 가림막도 각 위치에 둡니다.
...
| 가림막이 계속 위아래로 움직임 | `EEI_HYSTERESIS`, `CONTROL_STABLE_TIME_MS`, `MIN_POSITION_CHANGE_PERCENT`를 늘리고 센서가 햇빛/가림막 그림자에 의해 자기 피드백을 만드는 위치인지 확인합니다. |
